...
- For Agent Mode, connect with the **low-privilege investigation login** (see
  [ssms-database-constitution.md](ssms-database-constitution.md)).
...
```
Approve **only** read-only Query Store steps.
...
```
Cross-check against `scripts/sql/09-query-store-regression-report.sql`.
